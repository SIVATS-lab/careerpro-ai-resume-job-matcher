...
(function () {
    ...
    window.sendBotMessage = async function () {
        ...
        try {
            // Build the API URL dynamically from the current page's origin + path
            // This works regardless of subdirectory or domain — no hardcoded URLs needed
            const chatApiUrl = (function() {
                const loc = window.location;
                // Get the base path up to the project root (everything before the last segment)
                const pathParts = loc.pathname.split('/');
                pathParts.pop(); // remove current page filename
                const basePath = pathParts.join('/');
                return loc.origin + basePath + '/api/chat-handler.php';
            })();
