...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    if ($action === 'ai_assist') {
        ...
        // Try Gemini API first (supports both legacy AIzaSy... and new AQ. key formats)
        if (!empty($apiKey) && (str_starts_with($apiKey, 'AIzaSy') || str_starts_with($apiKey, 'AQ.'))) {
            ...
            foreach ($models as $model) {
                ...
                if ($httpCode === 400 || $httpCode === 401 || $httpCode === 403) break;
            }
        }
...
<body class="flex h-screen overflow-hidden selection:bg-pcte-500 selection:text-white">
    ...
    <script>
        ...
        // ============================================================================
        // ============================================================================
        // 10. PDF GENERATION ENGINE (HTML2PDF) — Fixed export matching preview
        // ============================================================================
        function exportPDF() {
            ...
            // Resolve the theme color CSS variable
            ...
            // We capture at 2x scale (1588 × 2246) for high resolution
            const A4_WIDTH_PX  = 794;
            const A4_HEIGHT_PX = 1123;
            const SCALE        = 2;

            // Collect all stylesheet text from the current page
            let allCSS = `:root { --theme-color: ${themeColor}; }
* { box-sizing: border-box !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
html, body { margin: 0 !important; padding: 0 !important; background: ${isTech ? '#111827' : 'white'} !important; }
.res-item { page-break-inside: avoid !important; break-inside: avoid !important; }
/* Force no transform on the paper element */
#pdf-clone-inner { transform: none !important; position: static !important; }`;

            document.querySelectorAll('style').forEach(s => { allCSS += '\n' + s.textContent; });
            // Replace CSS vars with real values
            allCSS = allCSS.replace(/var\(--theme-color\)/g, themeColor);
            // Remove any scale/transform rules that could shrink the content
            allCSS = allCSS.replace(/transform\s*:\s*scale\([^)]*\)[^;]*;/g, '');
            ...
            // Force exact A4 dimensions — NO transforms, no shadow, static position
            // padding: 20mm = ~75.6px at 96dpi, kept as-is to match preview
            clone.style.cssText = `
                width: ${A4_WIDTH_PX}px !important;
                min-height: ${A4_HEIGHT_PX}px !important;
                padding: ${isTech ? '0' : '75px 75px 75px 75px'} !important;
                box-shadow: none !important;
                overflow: visible !important;
                transform: none !important;
                background: ${isTech ? '#111827' : '#ffffff'} !important;
            ...
            // Remove zoom/transform from the wrapper if cloned
            const wrapper = clone.closest ? null : null; // wrapper is outside clone

            // Build a full standalone HTML document for the iframe
...
<style>
${allCSS}
/* Override any leftover Tailwind or layout styles that could clip the content */
#pdf-clone-inner { 
...
<body style="margin:0;padding:0;width:${A4_WIDTH_PX}px;background:${isTech ? '#111827' : 'white'};">
${clone.outerHTML}
</body></html>`;

            // Create a hidden iframe at exact A4 width — this prevents html2canvas from
            // measuring the element against a smaller container and clipping it
            const iframe = document.createElement('iframe');
            iframe.style.cssText = `position:fixed;top:-9999px;left:0;width:${A4_WIDTH_PX}px;height:${A4_HEIGHT_PX}px;opacity:0;pointer-events:none;border:none;z-index:-9999;`;
            ...
            iframe.onload = function() {
                ...
                if (!target) {
                    console.error('PDF: clone element not found in iframe');
                    document.body.removeChild(iframe);
                    btn.disabled  = false;
                    btn.innerHTML = originalText;
                    alert('PDF export failed — could not find resume element.');
                    return;
                }

                const opt = {
                    margin:   [0, 0, 0, 0],   // [top, left, bottom, right] in mm — no extra margin
                    filename: `${fullName}_Resume.pdf`,
                    image:    { type: 'jpeg', quality: 1.0 },
                    html2canvas: {
                        scale:           SCALE,       // 2x = crisp high resolution
                        useCORS:         true,
                        allowTaint:      true,
                        backgroundColor: isTech ? '#111827' : '#ffffff',
                        scrollX:         0,
                        scrollY:         0,
                        // Critical: set window & element width to EXACT A4 pixel width
                    ...
                    pagebreak: { 
                        mode:   ['css', 'legacy'],
                        avoid:  ['.res-item', '.res-section', '.res-header'],
                    },
                    jsPDF: { 
                        unit:        'mm', 
                ...
                // Use document.fonts.ready if available, else fall back to timeout
                if (iframeWin.document && iframeWin.document.fonts) {
                    iframeWin.document.fonts.ready.then(() => {
                        setTimeout(captureAndSave, 200);
                    });
                } else {
                    setTimeout(captureAndSave, 600);
                }
            };

            // Write into iframe — triggers onload
