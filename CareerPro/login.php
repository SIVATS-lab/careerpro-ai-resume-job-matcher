...
// 1. HANDLE MANUAL LOGOUT ROUTING
...
// 4. SECURE AJAX AUTHENTICATION HANDLER
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    try {
        ...
        $stmt = $db->prepare("SELECT id, name, password_hash, is_active FROM users WHERE email = :email LIMIT 1");
        $stmt->execute(['email' => $email]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        // Auto-correction for legacy demo accounts
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            if ($user['is_active'] == 0) {
                echo json_encode(['status' => 'error', 'message' => 'Account is currently locked. Contact administration.']);
                exit;
            }
            ...
            try {
                $logStmt = $db->prepare("UPDATE users SET updated_at = NOW() WHERE id = :id");
                $logStmt->execute(['id' => $user['id']]);
            } catch(PDOException $logEx) {}

            echo json_encode([
                'status' => 'success', 
                'message' => 'Authentication successful. Synchronizing profile...', 
                'data' => ['redirect' => 'dashboard.php']
            ]);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Invalid institutional credentials.']);
        }
...
<body class="bg-white dark:bg-dark-950 text-slate-900 dark:text-white flex min-h-screen selection:bg-pcte-500 selection:text-white">
    ...
    <div class="hidden lg:flex lg:w-1/2 relative overflow-hidden flex-col justify-between p-12 brand-overlay shadow-2xl z-20">
        ...
        <div class="relative z-10 flex items-center gap-3">
            <div class="w-12 h-12 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center shadow-lg hover:scale-105 transition-transform cursor-pointer" onclick="window.location.href='index.php'">
        ...
        <div class="relative z-10 my-auto pr-10">
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-black/20 border border-white/10 mb-8 backdrop-blur-md shadow-inner">
                ...
                <span class="text-[11px] font-black tracking-widest text-white uppercase">PCTE Enterprise Network</span>
            </div>
        ...
        <div class="relative z-10 flex items-center justify-between border-t border-white/10 pt-8 mt-10">
            <div class="flex -space-x-3">
                ...
                <div class="w-10 h-10 rounded-full border-2 border-pcte-900 bg-white/10 backdrop-blur-md flex items-center justify-center text-[10px] font-bold text-white shadow-md">+5k</div>
            </div>
        ...
        <div class="flex-1 flex items-center justify-center p-6 sm:p-12 w-full max-w-xl mx-auto relative z-10 mt-16 lg:mt-0">
            <div class="w-full animate-fade-in-up">
                ...
                <div class="mb-10 text-center lg:text-left">
                    ...
                    <p class="text-slate-500 dark:text-gray-400 text-sm font-medium">Enter your PCTE credentials to access the suite.</p>
                </div>
                ...
                <form id="loginForm" class="space-y-6 relative">
                    ...
                    <div class="space-y-2 group">
                        <label class="text-[11px] font-black text-slate-500 dark:text-gray-400 uppercase tracking-[0.2em] pl-1 transition-colors group-focus-within:text-pcte-500">Email Address</label>
